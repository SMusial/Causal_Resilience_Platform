...
### 29.1 Source hierarchy and project assumptions

The project uses the following source hierarchy:

* **Primary methodological source for V1:** Hernán and Robins, *Causal Inference: What If*. It is the methodological backbone, terminology reference, and syllabus anchor for the first release.
* **Supplementary conceptual sources:** Pearl and Cunningham may be used to clarify DAGs, structural causal models, counterfactual reasoning, intuition, and selected econometric methods. They supplement *What If*; they do not replace its estimand-first and target-trial framing.
* **Deferred specialist source:** VanderWeele is retained as a future reference, especially for mediation, interaction, and explanation of causal effects. It is not required for the V1 implementation because mediation is outside the V1 method scope.
* **Implementation references:** DoWhy, EconML, statsmodels, and scikit-learn documentation describe software APIs and implementation patterns. They are not treated as methodological authorities, and using a library does not establish causal identification or make an estimate valid.
* **Application and visualization references:** Streamlit and Plotly documentation support the course interface, sandbox, and interactive visualizations. They are engineering references, not causal-inference sources.
* **Project-architecture reference:** `rlvr-enterprise-allocator` is a reference for organizing an educational data-science project around one coherent operational use case. It is not a causal-inference source.

The following assumptions govern the project:

* V1 must remain **Python-first and Python-only**. R and Rust are deliberately excluded from the required release to keep the educational path, installation, testing, and maintenance coherent.
* Every causal result must be introduced through a causal question, estimand, target-trial-style specification, identification assumptions, and diagnostics before a library estimator is selected.
* The simulator's structural data-generating process and known ground truth are the primary validation oracle. Agreement with a software package is useful, but package output is not treated as proof of correctness.
* The project must distinguish clearly between a method being implemented, a method being demonstrated conceptually, and a method being deferred to a later release.
* Public documentation must not claim that all listed books or external sources were read in full when only a public webpage, documentation site, excerpt, or bibliographic record was available.
* The full *What If* PDF supplied with the project and the authors' official materials are considered available for the V1 methodological foundation. For other books, the project uses only the accessible material unless the full text is separately obtained and reviewed.

### 29.2 Methodological sources

* Hernán MA, Robins JM, [*Causal Inference: What If*](https://miguelhernan.org/whatifbook). **Primary V1 source.** Covers the core causal framework used by the project: potential outcomes, causal contrasts, exchangeability, positivity, consistency, confounding control, standardization/g-computation, inverse-probability weighting, longitudinal reasoning, and target-trial thinking. The book's own scope limitations are acknowledged; it is the backbone, not a claim of complete coverage of the entire causal-inference field.
* Pearl J, [*Causality*](https://bayes.cs.ucla.edu/BOOK-2K/). **Supplementary source.** Used selectively for DAGs, structural causal models, graphical identification, and counterfactual interpretation. The project does not assume access to the full book unless it is separately provided.
* Cunningham S, [*Causal Inference: The Mixtape*](https://mixtape.scunning.com/). **Supplementary source.** Used selectively for intuition, communication, and selected econometric implementations. Its examples may use R or Stata; V1 implementations remain Python-based.
* VanderWeele T, [*Explanation in Causal Inference*](https://global.oup.com/academic/product/explanation-in-causal-inference-9780199325870). **Deferred specialist source.** Reserved primarily for later mediation, interaction, and effect-decomposition work. The publisher page is a bibliographic reference, not evidence that the full text is available to the project.

### 29.3 Python implementation references

* [DoWhy documentation](https://www.pywhy.org/dowhy/). Reference for causal graphs, identification, estimation workflows, and refutation tooling. V1 must still expose the underlying assumptions and estimand instead of presenting DoWhy as a black-box causal engine.
* [EconML documentation](https://econml.azurewebsites.net/). Reference for heterogeneous treatment effects, double/debiased machine learning, treatment-effect validation, and later policy-learning extensions. Only methods explicitly included in the V1 scope are required for the first release.
* [statsmodels documentation](https://www.statsmodels.org/). Reference for transparent statistical models, regression, robust inference, and supporting analyses. It is not a dedicated causal-inference framework.
* [scikit-learn documentation](https://scikit-learn.org/). Reference for preprocessing, nuisance models, pipelines, model selection, and validation. It supplies building blocks; it does not perform causal identification.
* [Streamlit documentation](https://docs.streamlit.io/). Reference for the course mode, experimental sandbox, controls, and educational application layer.
* [Plotly Python documentation](https://plotly.com/python/). Reference for interactive causal visualizations, DAG-adjacent diagrams, effect plots, overlap diagnostics, distributions, and policy comparisons.

### 29.4 Project reference

* [SMusial/rlvr-enterprise-allocator](https://github.com/SMusial/rlvr-enterprise-allocator). **Educational architecture reference.** Informs the use of one coherent operational scenario, progressive chapters, reproducible experiments, baselines, visualizations, and interactive laboratories. It is not used as evidence for causal claims or as a substitute for causal-methodology sources.

### 29.5 V1 source policy

For every chapter and implementation module, the repository should identify:

* the relevant source and section;
* the causal question and estimand being taught;
* the assumptions required for identification;
* whether the implementation is educational, library-based, or both;
* the simulator ground truth and validation test;
* any material difference between the textbook method and the software implementation.

The V1 source set is intentionally small. Additional books, R reference implementations, Rust components, IV, mediation, survival, and advanced longitudinal methods may be added later only when they solve a documented V1 limitation or support a clearly scoped subsequent release.
